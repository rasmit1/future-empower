...
This framework combines three proven methodologies into a sequential diagnostic process for AI model performance issues. It is designed for AI leaders, transformation advisors, and program managers who need to diagnose problems without being data scientists.
...
**Layer 1 first** — the horizontal scan — maps the full problem landscape before going deep.
**Then Layer 2** — the 5 Whys vertical drill — finds the true root cause.
**Then Layer 3** — the Design Thinking check — ensures your fix will actually be adopted.

**The most important rule:** complete all three layers before recommending any fix. The most expensive mistake in AI diagnostics is solving the wrong problem — and the wrong problem almost always looks like the right problem at Layer 1.

---

## Layer 01 — The horizontal scan

*Who What When Where Why How — map the full landscape before you drill*

Before you diagnose anything you need a complete picture of the situation. Without this scan you risk running the 5 Whys on the wrong symptom.
...
**Layer 1 output:** After the horizontal scan you should be able to answer how many distinct problem types are present, who is affected by each, and what pattern the timing and location suggest. If you have more than one distinct symptom — run Layer 2 separately for each one. Do not combine them.

---

## Layer 02 — The 5 Whys vertical drill
...
### Example 2 — Vague results in different business unit
...
## Layer 03 — The Design Thinking check
...
- Has previous failure damaged adoption? How do we rebuild trust?
...
- What is the cost of the fix vs the cost of the problem?
...
**The adoption warning:** If users have already lost trust in the tool — the technical fix alone will not restore adoption. You need a trust-rebuilding plan alongside the technical fix.

---

## The diagnostic sequence — quick reference

1. **Resist the instinct to fix.** The first recommended fix is almost always wrong. Gather facts before prescribing solutions.
2. **Run the horizontal scan.** Map the full problem landscape. Identify how many distinct symptoms are present.
3. **Run 5 Whys separately for each symptom.** Do not combine symptoms. Each one may have a completely different root cause.
4. **Keep asking until you reach a human decision.** The true root cause is almost always a governance gap, ownership gap, design assumption, or process failure — not a model failure.
5. **Apply the Design Thinking check** to your proposed fix. All four dimensions must pass.
6. **Recommend immediate AND structural fix.** The immediate fix stops the bleeding. The structural fix prevents recurrence.
7. **Flag the regulatory implication.** In financial services every model performance issue has a potential regulatory dimension — FCA, CFPB, DORA, SR 11-7. Name it explicitly.

---

*Rashmi Mittal is an AI Transformation & Product Leader with 15+ years in financial services. This framework was developed from real enterprise AI engagements — it is designed to be used, not just read. 🌐 [futureempowered.com](https://futureempowered.com)*

`#AITransformation #AIDiagnostics #ModelPerformance #DesignThinking #5Whys #EnterpriseAI #AIGovernance #FutureEmpowered`
